...
description: "Lets build a component generator"
---

Building custom dev tools is very rewarding. Let's build a custom component
generator.

The CLI tool, we would be creating should be able to create component. We should
be able to customize the generated component with some options.
...
- Option to additional file like utility, test files.
- Option to choose the location where the component would be created.
- Ability to create the component from anywhere.

Okay let's start by creating a component. For templating we will be using
...
Let's create a folder called `src` and create a file named `index.js` in it.
Also create a folder named `templates` in `src`.

We would need `plop` as a dev dependency, `npm install plop -D`.

To run the component generator, we will add a npm script to in `package.json`.
`"create-component": "plop --plopfile src/index.js"`.

Okay, if you are looking for bunch of commands.
...
and if you provide the name of the component, it create a folder named after the
...
You may want add more options based on the project you are working on.
...
the filesystem. Something like `gen MyAwesomeButton`.

**Let's do that!** First we would have to create an executable script in
`bin/index.js`.
